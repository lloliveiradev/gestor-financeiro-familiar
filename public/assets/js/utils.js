...
function formValidate(form_id) {
    $(`#${form_id} .mandatory`).each((ix, el) => {
        if (['', null, undefined].includes($(el).val())) {
            const label = $(`label[for="${$(el).attr('name')}"]`).text()?.trim()?.replace(':', '');
            Swal.fire({
                title: 'Atention!',
...
function fetchAPI({
    ...
    if (method == "GET" && options.body) {
        delete options.body;
        return alert('GET request does not allow body parameters.');
    };
    callback = callback || function (res) { return res };
    return fetch((!path.includes("http") ? window.origin + path : (path)), options).then(async (res) => {
        if (file) {
        ...
        return res.json();
    }).then(callback).catch(async (error) => {
        throw await error;
    });
};